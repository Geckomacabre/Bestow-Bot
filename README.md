...
**[➕ Add Bestow to Discord](https://discord.com/oauth2/authorize?client_id=1553093201496375407)** — use the hosted bot right away, nothing to run yourself. Discord asks whether to add it to your account or to a server.
...
- **Add it to a server** as well to unlock the things Discord only lets a server-installed bot do: giveaways (which edit a message over hours or days), levels and XP, `/config` and `/server` settings, and other features that react to what happens in a channel. Those commands tell you when the bot isn't in the server.

To use the hosted Bestow, open **<https://discord.com/oauth2/authorize?client_id=1553093201496375407>** and pick your account or a server. You don't need to host anything.

If you run your own copy, `/invite` (and `/plus`) give both links for it, or you can build them from your application id:
...
| 🎁 **Giveaways** (`/giveaway`, `/eco giveaway`) | Button-entry giveaways that survive restarts, rerolls that don't pick anyone twice until everyone who entered has won, and coin-funded pots held in escrow (25% tax on payout, refunded if cancelled). |
...
| 🎬 **Media** (`/media`, `/audio`, `/download`) | Dozens of image, GIF and video effects, animated [MakeSweet](https://makesweet.com) scenes (billboard, flag, Rubik's cube, heart locket…, made by MakeSweet itself through its API — needs `MAKESWEET_API_KEY`), audio effects, frame extraction, `/gif`, `/voicemessage`, `/download` (YouTube, TikTok, X, Reddit, SoundCloud… via yt-dlp) and `/soundcloud`, plus song recognition with `/shazam` and the right-click **Identify Song** menu. |
| 🗣️ **Voice** (`/tts`) | Free local voices (Kokoro, with Microsoft Edge voices as the online fallback), character voices, singing voices, optional ElevenLabs voices, and optional AI singers through an ACE-Step server you run. Output as a file or a Discord **voice message**. |
| 🤖 **AI** (`/ai`, @mention, DMs) | ChatGPT-style answers with **Reply n/3** conversations, an alternative model (`/ai llama`), image generation and editing (`/ai imagine`, `edit-imagine`), Perplexity web search, OpenAI voices, your own **custom AI**, image reading, transcripts, summaries, fact-checks, geolocation (never addresses or homes), comedy bits (`/ai fun`), personas and opt-in memory; right-click **AI Tools**, **Transcribe Audio** and **Translate Message**. Server admins can turn @mention chat on or off with `/ai config`. Works with any OpenAI-compatible provider, including local ones. Free accounts get **20 requests per hour**; Premium removes the limit. |
| 🔎 **Lookups** | Games: `/roblox`, `/rolimons`, `/minecraft`, `/fortnite`, `/valorant`, `/steam`. Social: `/instagram`, `/tiktok`, `/x`, `/twitch`, `/youtube`, `/snapchat`, `/telegram`, `/pinterest`, `/medaltv`, `/cashapp`, `/bio`, `/github`, `/get` (profile pictures and banners). Music: `/lastfm`, `/spotify`, `/lyrics`. Crypto: `/crypto`, `/ton`. |
| 🧰 **Utility** | `/translate`, `/badtranslate`, `/define`, `/search`, `/qr`, `/color`, `/convert`, `/math`, `/base64`, `/paste`, `/dns`, `/ip`, `/website` (screenshots, scrolling videos, source downloads), `/bypass`, `/discord`, `/avatar`, `/banner`, `/tags`, `/timezone`, `/markov-chain`, `/ping`, and quote images with your own presets (`/quotemessage` and the right-click **Quote Message** menu); right-click **View Profile**. |
| 🎉 **Fun** | Anime action GIFs (`/action`), `/ship`, `/rate`, `/rating`, `/rizz`, `/roast`, `/petpet`, `/emojimix`, `/asciify`, `/say`, `/urban`, `/8ball`, `/coinflip`, `/nitro`, `/generate` (fake Discord messages, replies, conversations, reports, applications, friend requests and voice channels in Discord's themes and display-name fonts — each marked as not real — plus Among Us cards, Spotify lyrics, AI watermarks and a tier-list builder), a fictional `/juul`, `/button`, and right-click menus (Pet User, Rizz User, Roast User). |
| 🫂 **Community** (`/community`, `/config`, `/server`, `/pingonjoin`) | Levels and reputation, birthdays, reminders, self-assignable roles, weather, trivia and other small extras, free-game tracker, weekly/yearly rewards, mob protection and the item shop. Server settings cover counting, starboard, topic rotation, server stats and stat channels, streaming announcements, free-game posts, sticky messages, level roles, reaction roles, bulk role changes, the server economy and daily lottery, ping-on-join and the guessing-game channel; `/serverprofile` sets the bot's avatar, banner, bio and nickname in that server. |
| ✨ **Premium** (`/premium`) | Unlimited AI, no personal daily ElevenLabs limit, plus the commands marked ✨ in [COMMANDS.md](COMMANDS.md) (wallet-card styling, the monthly reward, video effects, `/ai imagine`, custom AIs, Instagram lookups, website downloads and more). Sold through Discord itself; gift codes and owner grants are supported. |
| ℹ️ **Info** | `/help`, `/about`, `/invite`, `/plus` (a second copy of the bot, if you run one), `/me`, `/donate`, `/customize`, `/getbotinvite`, `/serveravatar`, `/serverbanner`. |
...
- Some commands only work once their key or login session is set in `.env` (see **Lookup keys** and **Music** above). Without it they say what's missing.
