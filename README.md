...
| 🎬 **Media** (`/media`, `/audio`) | Dozens of image, GIF and video effects, animated [MakeSweet](https://makesweet.com) scenes (billboard, flag, Rubik's cube, heart locket…, made by MakeSweet itself through its API — needs `MAKESWEET_API_KEY`), audio effects, frame extraction, and `/download` (YouTube, TikTok, X, Reddit, SoundCloud… via yt-dlp). |
